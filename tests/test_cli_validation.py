...
class TestCliValidation:
    ...
    def test_validate_pdf_file_valid_extension(self):
    ...
    def test_validate_output_directory_nonexistent_fails(self, tmp_path):
    ...
    def test_validate_output_directory_current_dir_passes(self):
        """Test output directory validation passes for current directory"""
        output_path = Path("output.txt")
        # Should not raise any exception
        validate_output_directory(output_path)

    def test_validate_output_directory_existing_dir_passes(self, tmp_path):
    ...
    def test_validate_output_directory_file_not_dir_fails(self, tmp_path):
    ...
    def test_cli_validates_pdf_extension(self):
        """Test CLI rejects non-PDF files"""
        with tempfile.NamedTemporaryFile(suffix=".txt") as temp_file:
            with patch("shutil.which", return_value="/usr/bin/pdftk"):
                result = self.runner.invoke(app, ["dump", temp_file.name])
                assert result.exit_code != 0
                assert "must have .pdf extension" in result.stderr

    def test_cli_validates_file_exists(self):
        """Test CLI rejects non-existent files"""
        with patch("shutil.which", return_value="/usr/bin/pdftk"):
            result = self.runner.invoke(app, ["dump", "nonexistent.pdf"])
            assert result.exit_code != 0
            assert "does not exist" in result.stderr.lower()

    def test_cli_validates_output_directory_exists(self, tmp_path):
    ...
    def test_cli_accepts_existing_output_directory(self, tmp_path):
        ...
        with patch("shutil.which", return_value="/usr/bin/pdftk"):
            with patch("subprocess.run") as mock_subprocess:
                with patch(
                    "pdftoceditor.pdftoceditor.toc_from_metadata", return_value=[]
                ):
                    result = self.runner.invoke(
                        app, ["dump", str(test_pdf), "--output-toc", str(output_path)]
                    )
